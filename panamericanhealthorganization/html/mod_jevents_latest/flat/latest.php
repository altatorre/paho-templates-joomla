...
class FlatModLatestView extends DefaultModLatestView
{
	...
	function displayLatestEvents(){
		...
		if(isset($this->eventsByRelDay) && count($this->eventsByRelDay)){
			...
			$content .= $this->modparams->get("modlatest_templatetop") ? $this->modparams->get("modlatest_templatetop") : '<ul class="events-list">';
			...
			foreach($this->eventsByRelDay as $relDay => $daysEvents){
				...
				foreach($daysEvents as $dayEvent){
					...
					if($firstTime) $eventrow = '<li>%s'."</li>\n";
					else $eventrow = '<li>%s'."</li>\n";
			...
			$content .=$this->modparams->get("modlatest_templatebottom") ? $this->modparams->get("modlatest_templatebottom") : "</ul>\n";
		}
		...
		$content .= '<a href="'. JRoute::_("index.php?option=" . JEV_COM_COMPONENT . "&Itemid=" . $this->myItemid . "&task=" . $task . $this->catout, true) .'" class="more">'. JText::_('View more') .'</a>';
