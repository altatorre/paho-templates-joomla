...
function YOOMenuDropdownDecorator(&$node, $args) {
	...
	if ($node->name() == 'li') {
		...
		if (isset($node->ul[0]) && $level == 1) {
			$remove = array();
			$count  = count($node->ul[0]->_children);
			$column = 0;
			$row    = 0;
			...
			foreach ($node->ul[0]->_children as $i => $child) {
				...
				if ($row >= $rows) {
					$column++;
					$row  = 0;
					$rows = ceil(($count - $i) / ($columns - $column));
				}
				$row++;

				// add first column class attribute
				if ($i == 0) {
					$node->ul[0]->addAttribute('class', 'col'.($column + 1));
				}

				// add other columns class attribute and add children
				if ($column > 0) {
					if (!isset($node->ul[$column])) {
						$node->addChild('ul', array('class' => 'col'.($column + 1)));
					}
