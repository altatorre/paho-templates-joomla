...
function plgContentCreateTOC( &$row, &$matches, &$page )
{
	...
	// TOC Header
//	$row->toc = '<table style="padding:0px;border-collapse:collapse;border-spacing:0px" class="contenttoc"><tr><th>'.JText::_( 'Article Index' ).'</th></tr>';
	$row->toc = '	<div class="contenttoc"><ul style="list-style-type:none;margin:0">';

	// TOC First Page link
	$class = ($limitstart === 0 && $showall === 0) ? 'toclink active' : 'toclink';
//	$row->toc .= '<tr><td><a href="'. JRoute::_( '&showall=&limitstart=') .'" class="'. $class .'">'. $heading .'</a></td></tr>';
	$row->toc .= '<li style="display:inline;margin-right:7px"><a href="'. JRoute::_( '&showall=&limitstart=') .'" class="'. $class .'">' . $heading . '</a></li>';
...
//		$row->toc .= '<tr><td><a href="'. $link .'" class="'. $class .'">'. $title .'</a></td></tr>';
		$row->toc .= ' - <li style="display:inline;margin:0 7px"><a href="'. $link .'" class="'. $class .'">'
				. $title . '</a></li>';
		$i++;
	}
...
//	$row->toc .= '</table>';
	$row->toc .= "</ul></div><!-- end of .contenttoc -->\n\n";
}
...
function plgContentCreateNavigation( &$row, $page, $n )
{
	...
	// Cambio de Paulo Leite para mejorar la visibilidad de los pagebreaks. 
$l = '<ul class="'.$page.'" style="display:inline;margin: 0;list-style-type:none">';
for ($i=0;$i < $n;$i++) {
	if ($i == $page) {
		$l .= '<li style="margin:0 4px;display:inline-block;padding:2px 5px;border:1px solid #999"><a style="color:#bbb" href="' . JRoute::_('&limitstart='. ($i)) . '">' . ($i+1) . '</a></li>';
	} else {
		$l .= '<li class="'.$i.'" style="margin:0 4px;display:inline-block;padding:2px 5px;border:1px solid #999"><a href="' . JRoute::_('&limitstart='. ($i)) . '">' . ($i+1) . '</a></li>';
	}
}
$l .= '</ul>';

	$row->text .= '<div>' . $prev . ' - ' . $l . ' - ' . $next .'</div>';
}
