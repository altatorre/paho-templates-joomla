...
function currentDir() {
	...
	//echo "guardado";
		...
		$q = urldecode($this->searchword); 
		$cx = $this->cx;
		$sa = $this->sa;
		$cof = urlencode($this->cof);
		$searchphrase = urldecode($this->searchphrase);
		$ie = $this->ie;
		$scope = $this->scope;
		$option = $this->option;
		$Itemid = $this->Itemid;
		...
		$urlgoogle .= '#gsc.tab=0&gsc.q=' . $q . '&gsc.page=1';
			...
			//echo "<a href=\"index.php?Itemid=". $ai."\">" . JText::_('ADVANCED SEARCH') . "</a><br />";
		...
		if ( /* ($scope == 1) || ($scope == 2) || */ ($scope == 3)) { // para incluir a la OPS, incluir (($scope == 1) || ($scope == 2))
			...
			//  a través de Google
				// fue necesario aplicar una codificación utf-8 para que entendiera los acentos en español.
			$urlgoogle = utf8_encode( filter_var($urlgoogle, FILTER_SANITIZE_STRING) );
			$context = stream_context_create(array('http'=>array('timeout'=>10)));
			$urlgoogle = Reemplazar($urlgoogle, ' ', '%20');
			...
			if ($file_handle != null) { 
				...
				$mystring = Reemplazar($mystring, '<head>', '');
				$mystring = Reemplazar($mystring, '<meta http-equiv="content-type" content="text/html; charset=UTF-8" />', '');
				$mystring = Reemplazar($mystring, '<title>Google Custom Search</title>', '<title>Search:'.$q.'</title>');
				$mystring = Reemplazar($mystring, '</body>', '');
				$mystring = Reemplazar($mystring, '</html>', '');
