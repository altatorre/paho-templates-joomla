 ...
 * http://www.phpclasses.org/browse/file/28851.html
 */
...
class phpSEO{ 
    ...
    public static function getMetaDescription($len=null){//returns a text with meta description
	$this->setDescriptionLen($len);//by param set len
	$text=$this->getText();
//	if(strlen($text) getDescriptionLen($this->charset);
	if(mb_substr($text,mb_strlen($description),1)!=' ')
	return $this->getMetaDescription($len+1);
	return $description;
     }
