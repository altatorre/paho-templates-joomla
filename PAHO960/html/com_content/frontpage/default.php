...
<?php if ($this->params->def('num_leading_articles', 1)) : ?>
	<?php for ($i = $this->pagination->limitstart; $i < ($this->pagination->limitstart + $this->params->get('num_leading_articles')); $i++) : ?>
		...
		<div id="leading" style="width:100%;float:left">
		<?php
...
if($numintro) {
	echo "<div id=\"intro\" style=\"width:100%;float:left;border-top:0px solid #DDD;margin-top: -12px;padding-top:8px;margin-bottom:12px;background: url(templates/PAHO2014/images/div_line$numcolumns.png);background-repeat:repeat-y;background-position:center\">\n";
}

if ($multicolumnorder) {
for ($z = $numleading; $z < ($numleading+$numintro); $z++) {
	if ($z % $numcolumns) {
	echo "<div class=\"1\" style=\"width:$clwdt%;float:right;margin-left:$marg%\">\n";
	} else {
	echo '<div class="2" style="width:'.$clwdt.'%;float:left;margin-right:0">';
	}
	...
	if (($z % $numcolumns)) {
	echo "<div class=\"divider$z\" style=\"clear:both;padding-bottom:2px;border-bottom:1px solid #CCC\"></div>\n";
	}
}
	...
	for ($x = 1; $x <= $numcolumns; $x++) { // As many columns as needed
		if ($x < $numcolumns) {
			echo "<div class=\"1\" style=\"width:$clwdt%;float:left;margin-right:$marg%\">\n";
